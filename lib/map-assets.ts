 ...
 *
 * Server and script code only: `lib/data.ts` and the script. The client gets
 * `MapAssets`, a few strings and nine bounding boxes.
 */
...
export interface MapAssets {
  routesUrl: string;
  toursUrl: string;
  /** Per tour slug, only for tours with a routed geometry. */
 ...
 * (`<pass-slug>:<index>`). The properties are what the popup and the click
 * handler in `pass-map.tsx` read; status and selection come as feature state.
 */
...
const assetFile = (kind: "routes" | "tours", c: Collection): AssetFile => {
  ...
  return { body, name: `${kind}.${hash}.geojson` };
};
...
export const mapAssets = (
  ...
  for (const t of tours) {
    const geom = routes[`tour:${t.slug}`];
    if (geom) tourBounds[t.slug] = bounds(geom);
  }
  return {
    assets: {
