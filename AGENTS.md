...
1. **Stay static.** Content data lives in `data/*.json` and
   ...
   cached by name.
2. **German in the UI, and English under `/en`**; English in code, comments
   and docs. German is the source: every word the interface says is written
   in `lib/i18n/messages.de.ts` and the English file is held to its shape, so
   a missing translation is a type error. Numbers are formatted in the page's
   locale (`fmt` in `lib/utils.ts` takes the language, `useT()` binds it).
...
| The core · what the map draws                      | `buildScene` in `lib/map-scene.ts` and `pick` in `lib/map-pick.ts`, both read off the layer table `lib/layer-ids.ts`                                                                                                                                                                                                                                                                                                                                |
...
| Adapter · the address bar                          | `lib/hash-adapter.ts` (`useHashAdapter`, `cameraIntent`): the path and the hash in as the `load` action, the selection out as `router.push`, the rest as `history.replaceState`; the paths themselves `lib/routes.ts` (`SEGMENT`, `hrefFor`, `selectionOf`)                                                                                                                                                                                         |
| Adapter · web storage                              | `lib/use-stored.ts` (`PREFIX`, `STORAGE`, `useStored`, `useStorageAdapter`, `readStoredState`)                                                                                                                                                                                                                                                                                                                                                      |
| Adapter · MapLibre                                 | `components/map/apply-scene.ts` (the scene's difference), `components/map/apply-camera.ts` (the camera's commands) and `components/map/apply-environment.ts` (the environment's: base, overlays, scheme, terrain, the corner controls), all three driven from `components/map/pass-map.tsx`                                                                                                                                                         |
| Adapter · the fetch                                | `lib/use-fetch.ts`, read by `lib/detail-state.ts` for the detail file (the weather arrives with the route, `components/panel/weather.tsx`)                                                                                                                                                                                                                                                                                                          |
...
| The calendar: months, the 24 half-months           | `lib/period.ts` (`PERIODS`, `periodLabel`, `periodIndex`, `periodAt`, `isPeriod`, `todayPeriod`), vocabulary only – the heuristic is one of its readers                                                                                                                                                                                                                                                                                             |
...
| Tag labels: vocabulary, icons, badges              | `lib/regions.ts` (`TOWN_TAGS`, `ROAD_TAGS`), the labels `tagLabel` (`lib/i18n`), `lib/tag-icons.ts`, `components/tags.tsx`                                                                                                                                                                                                                                                                                                                          |
| The two languages: the words, the prefix, the menu | `lib/i18n/` (`LANGS`, `langPrefix`, `langOfPath`, `preferredLang`, `fill`; the sections `de/*.ts`, `en/*.ts`; `dictionaries.ts` and `getDictionary` in `server.ts` for the server), `components/i18n.tsx` (`I18nProvider`, `useT`), `components/map/language-field.tsx` with `switchLangHref` (`lib/hash-adapter.ts`), `proxy.ts`, the rewrites and redirects in `next.config.ts`; the curated prose `data/i18n/en/*.json`, merged in `lib/data.ts` |
...
| Weather, Open-Meteo quota and cooldown             | `lib/weather.ts` (`forecast`), streamed by `components/panel/weather.tsx` into the pass route's slot (`WeatherSlot`)                                                                                                                                                                                                                                                                                                                                |
| Entity routes, their metadata and share images     | `app/[lang]/(explorer)/layout.tsx` (the explorer, `"use cache"`), `app/[lang]/(explorer)/[kind]/[slug]/page.tsx` (+ `opengraph-image.tsx`), `lib/share-text.ts` (`openGraphOf`), `lib/share-image.tsx` (`ShareCard`), `staticParams`/`entityAt` in `lib/data.ts`, the alternates `alternatesOf` in `lib/routes.ts`                                                                                                                                  |
...
| Level of detail on the map (by fame)               | `lib/prominence.ts`; the filters in `components/map/app-layers.ts`, the corner line in `components/map/apply-environment.ts`                                                                                                                                                                                                                                                                                                                        |
...
| Base verdict, ranked reach, the inverse            | `lib/destination.ts` (`baseOf`, `basesOf`, `gradeOfBase`), `components/panel/base.tsx`, the calibration `scripts/analyze-destinations.ts` (`bun run analyze:destinations`)                                                                                                                                                                                                                                                                          |
| Destinations: the areas, membership, the score     | `data/destinations.json` (rules: `docs/destinations.md`), `membersOf`/`areaVerdict`/`areaScore` in `lib/destination.ts`, rows `buildDestinationRows` (`lib/rows.ts`), `components/sidebar/destination-list.tsx`, `compare-sheet.tsx`, `components/panel/destination-detail.tsx`; the outlines `DestinationMembers.outline`, drawn up to `DESTINATION_MAX_ZOOM` (`lib/prominence.ts`)                                                                |
...
| Photos: keys, sizes, licence metadata              | `lib/photos.ts`, `scripts/build-photos.ts` (`bun run data:photos`) → `data/generated/photos.json`; what a run chooses, borrows and re-blurs is `scripts/lib/photo-pipeline.ts`                                                                                                                                                                                                                                                                      |
...
| Icons, share image, manifest, robots, sitemap      | `app/icon.tsx`, `app/apple-icon.tsx`, `app/[lang]/opengraph-image.tsx`, `app/manifest.ts`, `app/robots.ts`, `app/sitemap.ts`                                                                                                                                                                                                                                                                                                                        |
| Legal pages                                        | `app/[lang]/impressum/`, `app/[lang]/datenschutz/` (German only, with an English note), their frame and metadata `components/legal-page.tsx`, the 404 `app/[lang]/not-found.tsx`                                                                                                                                                                                                                                                                    |
...
| [`docs/destinations.md`](docs/destinations.md)     | what a destination is, what its circle decides, how it is drawn                       |
...
- **Every word has one home, and it is not the component.** A sentence the
  app says is a key in `lib/i18n/messages.de.ts`, a plain string whose
  `{placeholders}` `fill` fills and the type checks; a function of the core
  that says something takes the words as its last argument (`w: Messages`).
  A component asks `useT()`, which the layout feeds one language's words;
  only the server, the scripts and the tests import a dictionary.
  → [why](docs/ui-conventions.md#every-word-has-one-home-and-it-is-not-the-component)
...
- **One list at a time, chosen by a tab row.** Roads, areas and loops sit
  behind tabs in the fixed header, the towns under their area; each list's
  map switch rides in its own toolbar.
...
- **Cache Components.** `"use cache"` on `app/[lang]/(explorer)/layout.tsx` and
  nowhere else on the page's path: `lib/data.ts` is synchronous, and the
  layout's entry covers what it derives; the entity routes under it fill a
  slot. `cookies()`, `headers()` and `searchParams` belong in a dynamic
  child.
...
- **The one dynamic route lives inside a free tier, and the numbers are in the
  file.** The forecast (`lib/weather.ts`), streamed into the pass route behind
  `connection()`: an hour-long window, never part of a prerender, and a
  cooldown inside the cached function.
...
`bun run test` runs the unit tests next to the code (what CI runs; a bare
`bun test` would also pick up the e2e suite); `bun run e2e` drives the built
app in a headless browser (needs a Chrome, see `test/browser.ts`).
