...
| Entity routes, their metadata and share images  | `app/(explorer)/layout.tsx` (the explorer, `"use cache"`), `app/(explorer)/[kind]/[slug]/page.tsx` (+ `opengraph-image.tsx`), `lib/share-text.ts`, `lib/share-image.tsx`, `staticParams`/`getEntity` in `lib/data.ts`                                                                                                                      |
...
- **Cache Components.** `"use cache"` on `app/(explorer)/layout.tsx` and
