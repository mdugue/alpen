...
| Filter, selection and URL state (hash keys)     | `lib/app-state.ts` (`reduce`, `initialState`, `Shown`), `entityKey` in `lib/route-key.ts`, the hash keys in `lib/hash.ts`, the adapters `lib/hash-adapter.ts` (`useHashAdapter`) and `lib/use-stored.ts` (`STORAGE`, `useStorageAdapter`, `readStoredState`); dispatched from `components/explorer.tsx`                                    |
