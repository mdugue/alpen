...
| The two languages: the words, the prefix, the toggle | `lib/i18n/` (`LANGS`, `langPrefix`, `langOfPath`, `messagesOf`, `vocabOf`; the sections `de/*.ts`, `en/*.ts`), `components/i18n.tsx` (`I18nProvider`, `useT`), `switchLangHref` in `lib/hash-adapter.ts`, the rewrites in `next.config.ts`; the curated prose `data/i18n/en/*.json`, merged in `lib/data.ts`                               |
...
| Destinations: the areas, membership, the score       | `data/destinations.json` (rules: `docs/destinations.md`), `membersOf`/`areaVerdict`/`areaScore` in `lib/destination.ts`, rows `buildDestinationRows` (`lib/rows.ts`), `components/sidebar/destination-list.tsx`, `compare-sheet.tsx`, `components/panel/destination-detail.tsx`; the circles `DESTINATION_MAX_ZOOM` (`lib/prominence.ts`)  |
...
- **Every word has one home, and it is not the component.** A sentence the
  app says is a key in `lib/i18n/messages.de.ts`, an interpolation is a
  function there, and a function of the core that says something takes the
  language as its last argument (German by default, so the scripts and the
  tests keep their words). A component asks `useT()`; nothing under `lib/`
  reads the provider.
