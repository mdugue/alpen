...
| Linting, formatting and the seam check          | `oxlint.config.ts` (the `lib/**` seam rules and the adapter allow-list), `oxfmt.config.ts`, `scripts/check-seams.ts` (`bun run seams`, inside `bun run lint`)                                                                                                                                                                              |
