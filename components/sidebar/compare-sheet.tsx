  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] grid-rows-[auto_minmax(0,1fr)] sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>{t.sidebar.compare.title}</DialogTitle>
          <DialogDescription>{t.sidebar.compare.description}</DialogDescription>
        </DialogHeader>
        <div className="overflow-y-auto pr-1">
          {rows.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              {t.sidebar.compare.none}
            </p>
          ) : (
            <div
                ...
                return (
                  ...
                  >
                    ...
                    <dl className="text-muted-foreground grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 tabular-nums">
                      <dt>{t.sidebar.compare.roads}</dt>
                      <dd className="text-foreground">
                        {fmt(row.members.passes.length)}
                      </dd>
                      <dt>{t.sidebar.compare.loops}</dt>
                      <dd className="text-foreground">
                        {fmt(row.members.tours.length)}
                      </dd>
                      <dt>{t.sidebar.compare.towns}</dt>
                      <dd className="text-foreground">
                    ...
                    {row.baseTowns.length > 0 && (
                      <p>
                        <span className="text-muted-foreground">
                          {t.sidebar.compare.baseTowns}
                        </span>
                        {row.baseTowns.map((town) => town.name).join(", ")}
                      </p>
                    )}
