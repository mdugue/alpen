...
function SectionTitle({
  ...
  return (
    <h3 className="border-border text-muted-foreground mt-5 mb-2 flex items-baseline gap-2 border-b pb-1 text-[11px] font-semibold tracking-widest uppercase">
      {children}
      {hint && (
        <span className="text-[11px] font-normal tracking-normal normal-case">
          {hint}
        </span>
      )}
    </h3>
    ...
    <>
      <div className="mt-2 mb-2 flex flex-wrap items-baseline gap-3">
        <span className="text-4xl leading-none font-bold tabular-nums">
          {fmt(pass.elevation)}
          <span className="text-muted-foreground ml-1 text-lg">m</span>
        </span>
      ...
      <dl className="my-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-[13px]">
        ...
        <dd className="text-muted-foreground grid grid-cols-[auto_auto] items-center gap-x-3 gap-y-0.5 justify-self-start text-xs">
          <span>Schönheit</span>
      ...
      <SectionTitle hint="ERA5-Land, 2015–2024">
        Klima auf Passhöhe
      </SectionTitle>
      {bucket && climate ? (
        <>
          <p className="text-muted-foreground mb-1.5 text-xs">
            ...
            Mittel 2015–2024 (≈ 15 Tage je Halbmonat):
          </p>
          <div className="grid grid-cols-3 gap-1.5">
            {[
              [
                `${fmt(bucket.tmax)} / ${fmt(bucket.tmin)} °C`,
                "Ø Höchst-/Tiefstwert",
              ],
              [
                `${bucket.frostPct} %`,
                `Frost (≈ ${days(bucket.frostPct)} von 15)`,
              ],
              [
                `${bucket.snowPct} %`,
                `Schneefall (≈ ${days(bucket.snowPct)} von 15)`,
              ],
            ].map(([value, label]) => (
          ...
          <p className="text-muted-foreground mt-1.5 text-xs">
            Niederschlag ≥ 1 mm an {bucket.wetPct} % der Tage (≈{" "}
            {days(bucket.wetPct)} von 15).
          </p>
          <ClimateChart climate={climate} period={props.period} />
          <p className="text-muted-foreground mt-1 text-xs">
            ERA5-Land ist ein 10-km-Raster und auf Passhöhe eher zu mild – gut
            zum Vergleich der Zeiträume, nicht als Absolutwert.
          </p>
        </>
      ) : (
        <Empty className="py-3">
    ...
    <>
      <div className="mt-2 mb-2 flex flex-wrap items-baseline gap-3">
        <span className="text-3xl leading-none font-bold tabular-nums">
          {fmt(tour.km)}
          <span className="text-muted-foreground ml-1 text-base">km</span>
        </span>
        <span className="text-3xl leading-none font-bold tabular-nums">
          {fmt(tour.elevationGain)}
          <span className="text-muted-foreground ml-1 text-base">hm</span>
        </span>
        <StatusBadge status={status} period={props.period} />
      </div>
      <SeasonStrip
