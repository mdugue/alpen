...
import { ascentMetrics, checkAscent, checkSummit, checkTour, tourMetrics, withProfile } from "./lib/validate";
...
const rejected = await read<Record<string, RouteRejection>>("rejected.json", {});
...
const days = (iso: string) => Math.round((Date.now() - Date.parse(iso)) / 86_400_000);

/** Shared by ascents and tours: judge what is stored, and note where it came from. */
function inspect(key: string, label: string, metrics: RouteMetrics, reasons: string[]) {
  const source = meta[key]?.source ?? "osrm";
  if (reasons.length)
    errors.push(`${key}: gespeicherte Route ist unplausibel – ${reasons.join("; ")} (${label})`);
  else if (source === "osrm")
    warnings.push(`${key}: Route stammt vom OSRM-Autoprofil – mit ORS_KEY erneuern`);
  if (EXPLAIN)
    explained.push(
      `${reasons.length ? "✗" : "·"} ${key.padEnd(36)} ${source.padEnd(4)} ${JSON.stringify(metrics)}` +
        (reasons.length ? `\n      ${reasons.join("\n      ")}` : ""),
    );
}
...
for (const p of passes as Pass[]) {
  ...
  if (dem === undefined) warnings.push(`${p.slug}: Gipfelhöhe ungeprüft (bun run data:build)`);
  else
    for (const r of checkSummit(dem, p.elevation))
      warnings.push(`${p.slug}: ${r} – Passkoordinate prüfen (DEM ${dem} m, angegeben ${p.elevation} m)`);

  p.ascents.forEach((a, i) => {
    const key = `${p.slug}:${i}`;
    if (a.check && !a.check.note?.trim()) errors.push(`${key}: check ohne Begründung (note)`);
    const geom = routes[key];
    if (!geom) {
      if (!(key in rejected)) warnings.push(`${key}: Route fehlt (bun run data:build)`);
      return;
    }
    ...
    inspect(key, `${p.name} ab ${a.label}`, m, checkAscent(m as AscentMetrics, a.check));
  });
...
for (const t of tours as Tour[]) {
  ...
  if (t.check && !t.check.note?.trim()) errors.push(`${key}: check ohne Begründung (note)`);
  const geom = routes[key];
  if (!geom) {
...
for (const [key, r] of Object.entries(rejected)) {
  ...
  if (EXPLAIN) explained.push(`✗ ${key.padEnd(36)} ${r.source.padEnd(4)} ${JSON.stringify(r.metrics)} (abgewiesen)`);
}

if (EXPLAIN) {
  console.log("Gemessene Werte je Route (✗ = verletzt eine Grenze aus scripts/lib/validate.ts):");
  for (const line of explained.sort()) console.log(`  ${line}`);
  console.log();
}
